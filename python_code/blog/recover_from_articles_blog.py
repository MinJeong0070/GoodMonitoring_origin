...
from python_code.core_utils import clean_text, calculate_copy_ratio, log

# 날짜
today = datetime.now().strftime("%y%m%d")

# 경로 설정
input_path = f"../../전처리/블로그/4월/전처리_네이버블로그_4_0406-0420_250526.xlsx"
article_folder = f"../../결과/기사본문_{today}"
output_path = f"../../결과/원문기사_네이버블로그_1_{today}.xlsx"
...
for filename in files:
    try:
        ...
        title = clean_text(str(df.at[index, "게시글제목"]))
        content = clean_text(str(df.at[index, "게시글내용"]))
...
stats_rows = pd.DataFrame([
    ...
    {"순번": "0.3 이상", "검색": f"{above_30_count}건"},
    {"순번": "0.8 이상", "검색": f"{above_80_count}건"},
])
